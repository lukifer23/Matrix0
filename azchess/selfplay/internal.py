...
def selfplay_worker(proc_id: int, cfg_dict: dict, ckpt_path: str | None, games: int, q: Queue | None = None,
    ...
    mcts = MCTS(
        ...
        MCTSConfig(
            ...
            selection_jitter=float(sp_cfg.get("selection_jitter", 0.01)),
            batch_size=int(sp_cfg.get("batch_size", 32)),
            fpu=float(sp_cfg.get("fpu", 0.5)),
            parent_q_init=bool(sp_cfg.get("parent_q_init", True)),
        ),
        device=device,
