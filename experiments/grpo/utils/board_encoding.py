...
def board_to_tensor(board: chess.Board, device: torch.device = None) -> torch.Tensor:
    ...
    for piece_type in range(1, 7):  # 1-6: pawn, knight, bishop, rook, queen, king
        white_channel = torch.zeros(8, 8, device=device)
        black_channel = torch.zeros(8, 8, device=device)
    ...
    # Side to move
    side_to_move = torch.ones(8, 8, device=device) if board.turn == chess.WHITE else torch.zeros(8, 8, device=device)
    channels.append(side_to_move)

    # Castling rights
    castling_channels = [torch.zeros(8, 8, device=device) for _ in range(4)]
    ...
    # En passant square
    en_passant = torch.zeros(8, 8, device=device)
    ...
    halfmove = torch.full((8, 8), min(board.halfmove_clock / 100.0, 1.0), device=device)
